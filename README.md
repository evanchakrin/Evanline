...
| **Caster (camber swing)** | Caster is **not** a static gravity reading — it is recovered from how **camber** changes as the wheel is steered. Hold the phone flush to the wheel face (camber pose), steer a known angle θ each way, and capture a **settled** camber read at each position; `caster = (camber_out − camber_in) / (2·sin θ)`. Because it is a **difference** of two camber reads, the camber zero cancels — **no zeroing step**. Each read's ±band is propagated (≈2× the per-read camber band at 20°). The **magnitude is robust**, but the absolute **sign must be validated against a known-caster reference**; honest accuracy ~±0.5–1° |
...
6. **Caster is a camber swing, not a static read.** The phone cannot sense caster directly — recover it from how **camber** changes between two steer positions. Use turn plates (or a printable degree disc) to steer a known angle θ each way, hold the phone in the **camber pose**, and capture a **settled** camber read steered **out** and steered **in**: `caster = (camber_out − camber_in) / (2·sin θ)` (classic θ = 20° each way). Because it is a **difference** of two camber reads the zero offset cancels, so caster needs **no zeroing step** — capture raw camber both ways. The propagated ±band is ≈2× the per-read camber band at 20°; honest accuracy is ~±0.5–1°. The **magnitude is robust, but validate the absolute sign against a known-caster reference** (positive caster = top of the steering axis tilted rearward).
7. **Read the band, not the digit.** Every saved value carries a `± Y.YY° (95%)` confidence band. Treat the band — not the single displayed digit — as the real result.
8. **Scale-calibrate against a known angle (optional).** After zeroing, if you have a machined reference wedge, tap **Set scale**, hold the phone on it, and enter the true angle. The app derives `gain = true / measured` and scales every later reading, correcting a sensor that under/over-reports. Left unset, the gain stays `1.00×` (no change).
